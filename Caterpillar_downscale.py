...
credit_font = ('calibri', 10)
help_font = ('calibri', 10)
elements_font = ('calibri', 5)
general_pady = 4
entry_width = 150
txt_color = '#C5F9C7'
txt_color_alt = 'black'
...
class Caterpillar:
    def __init__(self, jan):
        ...
        def mainscreen():
            ...
            largura_ambiente = ctk.CTkEntry(c_ambiente, placeholder_text='Largura (m)', font=credit_font, width=150, text_color=txt_color, fg_color=entry_bg)
            largura_ambiente.pack(pady=general_pady)
            comprimento_ambiente = ctk.CTkEntry(c_ambiente, placeholder_text='Comprimento (m)', font=credit_font, width=150, text_color=txt_color, fg_color=entry_bg)
            comprimento_ambiente.pack(pady=general_pady)
            altura_ambiente = ctk.CTkEntry(c_ambiente, placeholder_text='Altura (m)', font=credit_font, width=150, text_color=txt_color, fg_color=entry_bg)
            ...
            x_coord = ctk.CTkEntry(c_posicao, placeholder_text='X (m)', font=credit_font, width=150, text_color=txt_color, fg_color=entry_bg)
            x_coord.pack(pady=general_pady)
            y_coord = ctk.CTkEntry(c_posicao, placeholder_text='Y (m)', font=credit_font, width=150, text_color=txt_color, fg_color=entry_bg)
            y_coord.pack(pady=general_pady)
            z_coord = ctk.CTkEntry(c_posicao, placeholder_text='Z (m)', font=credit_font, width=150, text_color=txt_color, fg_color=entry_bg)
            ...
            desenhar.pack(pady=general_pady, padx=18)
            ...
            temp_frontal = ctk.CTkEntry(c_temp, placeholder_text='Frontal (°C)', font=credit_font, width=96, text_color=txt_color, fg_color=entry_bg)
            temp_frontal.grid(row=1, column=0, pady=general_pady, padx=general_pady)
            temp_lat_esq = ctk.CTkEntry(c_temp, placeholder_text='Lateral Esquerda (°C)', font=credit_font, width=96, text_color=txt_color, fg_color=entry_bg)
            temp_lat_esq.grid(row=2, column=0, pady=general_pady, padx=general_pady)
            temp_piso = ctk.CTkEntry(c_temp, placeholder_text='Piso (°C)', font=credit_font, width=96, text_color=txt_color, fg_color=entry_bg)
            temp_piso.grid(row=3, column=0, pady=general_pady, padx=general_pady)
            temp_posterior = ctk.CTkEntry(c_temp, placeholder_text='Posterior (°C)', font=credit_font, width=96, text_color=txt_color, fg_color=entry_bg)
            temp_posterior.grid(row=1, column=1, pady=general_pady, padx=general_pady)
            temp_lat_dir = ctk.CTkEntry(c_temp, placeholder_text='Lateral Direita (°C)', font=credit_font, width=96, text_color=txt_color, fg_color=entry_bg)
            temp_lat_dir.grid(row=2, column=1, pady=general_pady, padx=general_pady)
            temp_teto = ctk.CTkEntry(c_temp, placeholder_text='Teto (°C)', font=credit_font, width=96, text_color=txt_color, fg_color=entry_bg)
            ...
            calcular.pack(pady=general_pady, padx=18)
        ...
        def make_house(largura:float, altura:float, comprimento:float, x:float, y:float, z:float):
            cube_size = 12
            size_of_pen = 3
            if (largura >= 240) or (altura >= 240) or (comprimento >= 240):
                if (largura >= 600) or (altura >= 600) or (comprimento >= 600):
                    size_of_pen = 2
                greater = max([largura, altura, comprimento])
                largura = largura/(2*(greater/240))
                comprimento = comprimento/(2*(greater/240))
                altura = altura/(2*(greater/240))
                x = x/(2*(greater/240))
                y = y/(2*(greater/240))
                z = z/(2*(greater/240))
                cube_size = cube_size/(2*(greater/240))
            if (largura <= 60) or (altura <= 60) or (comprimento <= 60):
                largura = largura*1.5
        ...
        help_button = ctk.CTkButton(window, text='?', text_color=txt_color_alt, font=title_font, width=15, fg_color=txt_color, corner_radius=90, command=self.open_help)
        help_button.place(x=240,y=10.2)

        frame = ctk.CTkFrame(window, fg_color=foreground, width=222, height=520)
        frame.grid(row=1, column=0, padx=36)
        frame.pack_propagate(False)
        frame.grid_propagate(False)

        credit = ctk.CTkLabel(window, text='Zac Milioli, LabEEE 2024', text_color=txt_color, font=credit_font)
        credit.grid(row=2, column=0, pady=12)

        canvas = Canvas(window, width=588, height=576, highlightbackground=foreground)
        ...
        iniciar_app = ctk.CTkButton(frame, text='Iniciar', fg_color=iniciar_button, font=title_font, text_color=txt_color_alt, width=30, height=90, corner_radius=90, command=start_program)
        iniciar_app.pack(pady=180)
    ...
    def open_calculate_window(self, largura, altura, comprimento, x, y, z, temp_frontal, temp_lat_esq, temp_piso, temp_posterior, temp_lat_dir, temp_teto):
        ...
        calculation_window.geometry('360x640')
        ...
        titulo_results.pack(pady=12)

        topframe = ctk.CTkScrollableFrame(calculation_window, fg_color=foreground, height=550, width=288)
        topframe.pack(padx=18)
